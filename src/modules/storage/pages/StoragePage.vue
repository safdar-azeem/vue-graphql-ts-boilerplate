...
// Files Query
// If searching: Pass undefined to folderId to search ALL folders
// If browsing: Pass currentFolderId (null for root) to scope to current view
const {
  result: filesResult,
  ...
  filter: {
    // Crucial fix: When searching, remove folder constraint (undefined).
    // When browsing, enforce folder constraint (string or null).
    folderId: isSearching.value ? undefined : currentFolderId.value,
    search: isSearching.value ? searchQuery.value : null,
  },
}))
...
const {
  ...
  filter: {
    // Same logic for folders: Global search vs Scoped browse
    parentId: isSearching.value ? undefined : currentFolderId.value,
    search: isSearching.value ? searchQuery.value : null,
  },
}))
...
const handleFolderClick = (folder: any) => {
  // When navigating into a folder, clear search to enter "Browse Mode"
  searchQuery.value = ''
  currentFolderId.value = folder.id
  
  // Rebuild breadcrumbs if we are jumping from a global search result
  if (isSearching.value) {
    // If we were searching, we might be jumping into a deep folder.
    // For simplicity in this flat-search view, we just reset to that folder.
    // A robust app might fetch the folder path here.
    breadcrumbs.value = [
      { id: null, name: 'Home' },
    ...
    <div
      ...
      <div class="w-full sm:w-auto">
        ...
        <div v-else class="mt-1 text-sm text-primary font-medium flex items-center gap-1">
          ...
          <button @click="searchQuery = ''" class="ml-2 text-gray-400 hover:text-red-500 text-xs underline">
            Clear Search
          </button>
        </div>
      </div>
