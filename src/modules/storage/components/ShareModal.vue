...
import { Button, Icon, Form, Badge, showToast } from 'vlite3'
import {
...
const fileLinksEnabled = computed(() => props.item?.type === 'file' && !!props.item?.id)

const folderLinksEnabled = computed(() => props.item?.type === 'folder' && !!props.item?.id)
...
<template>
  <div class="space-y-6 py-2">
    <div class="bg-gray-50 p-4 rounded-lg border border-gray-100">
      <h4 class="text-sm font-medium text-gray-900 mb-3">Create New Link</h4>
      <Form
        :schema="[
          {
            name: 'expiresIn',
        ...
        <template #submit="{ loading }">
          <Button type="submit" :loading="loading" icon="lucide:link" class="w-full">
            Create Share Link
          </Button>
        </template>
      </Form>
    ...
    <div>
      <h4 class="text-sm font-medium text-gray-900 mb-3 flex items-center justify-between">
        ...
        <span v-if="activeLinks.length" class="text-xs text-gray-500"
          >{{ activeLinks.length }} active</span
        >
      </h4>

      <div v-if="fileLoading || folderLoading" class="flex justify-center py-4">
        <div
      ...
      <div
        ...
        <Icon icon="lucide:link-2-off" class="w-8 h-8 mx-auto mb-2 text-gray-300" />
        ...
        <div
          ...
          <div class="flex-1 min-w-0 mr-4">
            <div class="flex items-center gap-2 mb-1">
              <p class="text-sm font-medium text-gray-900 truncate max-w-[200px]" :title="link.url">
                {{ link.url }}
              </p>
              <Badge v-if="isExpired(link.expiresAt)" variant="danger" size="sm">Expired</Badge>
              <Badge v-else variant="success" size="sm">Active</Badge>
            </div>
