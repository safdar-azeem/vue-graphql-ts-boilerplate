...
<template>
  <div class="space-y-6">
    ...
    <div v-if="!token" class="p-3 rounded-md bg-yellow-50 text-yellow-700 text-sm text-center">
      Invalid or missing token.
    </div>

    <div v-else-if="success" class="p-4 rounded-md bg-green-50 text-green-700 text-sm text-center">
      Password reset successfully! Redirecting to login...
    </div>

    <div v-else>
      <div v-if="error" class="p-3 mb-4 rounded-md bg-red-50 text-red-600 text-sm text-center">
        {{ error.message }}
      </div>
