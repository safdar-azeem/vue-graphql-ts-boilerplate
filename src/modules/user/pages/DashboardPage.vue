  ...
  <div class="min-h-screen bg-gray-50 p-8">
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-sm border p-8">
      <div class="flex items-center justify-between border-b pb-6 mb-6">
        ...
        <Button variant="outline" icon="lucide:log-out" @click="handleLogout">Logout</Button>
      </div>
      ...
      <div v-else-if="result && result.me" class="space-y-6">
        <div class="flex items-center gap-4">
          <Avatar size="xl" :alt="result.me.username" />
          <div>
            <h2 class="text-xl font-semibold">{{ result.me.username }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="p-6 bg-gray-50 rounded-lg border">
            ...
            <ul class="text-sm text-gray-600 space-y-2">
              ...
              <li><strong>Joined:</strong> {{ new Date(result.me.createdAt).toLocaleDateString() }}</li>
            </ul>
          </div>
          ...
          <div class="p-6 bg-gray-50 rounded-lg border">
            ...
            <div class="flex items-center justify-between">
              ...
              <span 
                ...
                :class="result.me.mfaSettings?.isEnabled ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'"
              >
                {{ result.me.mfaSettings?.isEnabled ? 'Enabled' : 'Disabled' }}
              </span>
            </div>
          </div>
        </div>
