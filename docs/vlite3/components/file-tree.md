...
The indent guides are drawn by the shared `TreeConnectors` component (`src/components/TreeConnectors`), the same primitive used by `DataTable`'s tree mode. Every node renders a vertical trunk, a horizontal branch to its icon, and a corner when it is the last child — all computed dynamically from the node's depth, its position among its siblings (`isLast`), and its ancestors' last-child flags (`ancestorLastFlags`, threaded through the recursion). Nothing is hard-coded, so the guides stay correct at any nesting depth and update automatically as folders expand, collapse, or load asynchronously.
