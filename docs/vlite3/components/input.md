...
| `addonLeft`       | `string`                                               | —         | Text addon on the left (control strip is always LTR) |
| `addonRight`      | `string`                                               | —         | Text addon on the right (control strip is always LTR) |
...
The label follows page direction. The addon control strip is a **`dir="ltr"` island**: `addonLeft` stays on the left and `addonRight` on the right (URLs, currency, protocols, copy actions). Joined corners use logical radii (`rounded-s` / `rounded-e`) which match physical left/right inside that island. Nested Dropdowns with `direction="auto"` pick up the island `dir` so labels like `https://` do not reverse.

The text field itself (typed value, **placeholder**, leading/trailing icons, clear/password controls, and logical padding) follows page direction via a nested `.input-field-chrome` island, so Arabic/Hebrew placeholders and icons mirror correctly even when addons stay LTR.
