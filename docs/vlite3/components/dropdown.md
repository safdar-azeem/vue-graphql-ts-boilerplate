...
| `direction`          | `'ltr' \| 'rtl' \| 'auto'`                   | `'auto'`             | Text/layout direction. `auto` follows nearest ancestor `dir`, then `html[dir]`        |
...
By default `direction="auto"` follows the nearest ancestor `dir` (e.g. an Input addon `dir="ltr"` island), then `html[dir]`. Pass `direction="rtl"` / `"ltr"` to force a side.
