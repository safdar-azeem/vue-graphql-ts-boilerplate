...
- Nested indent and tree connectors use `margin-inline-start` / `inset-inline-start`
- Expand / drilldown chevrons point toward inline-end (`chevron-left` in RTL)
- Drilldown back control and slide transitions mirror with direction
- Side popovers / tooltips open toward the content (`left-start` / `left` in RTL)
